...
def outfile_create():
    ...
    if usr_input == "y":
        out_dir = main_file
        files_created = 0 
        # out_dir = input("Enter the path where you want to store these files (Leave blank to create file in current directory): ")
        # if not out_dir:
        #     out_dir = main_file
        # files_created = 0 
        # else:
        ...
        while files_created < num_of_files:
            new_file = f"{out_dir}-SPLIT0{files_created}"
            open(f"{new_file}", "w")
